...
def main() -> int:
    ...
    scenarios = [
        ...
        (
            "raw client with repeated permessage-deflate offers -> zwebsocket server",
            port + 4,
            [str(server_bin), f"--port={port + 4}", "--compression"],
            ["python3", str(VALIDATION_DIR / "repeated_pmd_offer_client.py"), f"--port={port + 4}"],
        ),
        (
            "zwebsocket client -> node-ws server",
            port + 5,
            ["node", str(VALIDATION_DIR / "ws_peer.mjs"), "server", f"--port={port + 5}"],
            [str(client_bin), f"--port={port + 5}"],
        ),
        (
            "zwebsocket client -> node-ws server (permessage-deflate)",
            port + 6,
            ["node", str(VALIDATION_DIR / "ws_peer.mjs"), "server", f"--port={port + 6}", "--compression"],
            [str(client_bin), f"--port={port + 6}", "--compression"],
        ),
        (
            "zwebsocket client -> aiohttp server",
            port + 7,
            ["python3", str(VALIDATION_DIR / "aiohttp_peer.py"), "server", f"--port={port + 7}"],
            [str(client_bin), f"--port={port + 7}"],
        ),
        (
            "zwebsocket client -> aiohttp server (permessage-deflate)",
            port + 8,
            ["python3", str(VALIDATION_DIR / "aiohttp_peer.py"), "server", f"--port={port + 8}", "--compression"],
            [str(client_bin), f"--port={port + 8}", "--compression"],
        ),
    ]
